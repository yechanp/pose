...
def write_json(all_results, outputpath, opt, form=None, for_eval=False):
    '''
    ...
    json_results_cmu = {}
    if len(opt.video):
        file_name = os.path.basename(opt.video)
    else:
        file_name = all_results[0]['imgname'].split('.')[0]
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            im_name_wo = os.path.basename(im_name).split('.')[0]
    ...
    if form == 'cmu': # the form of CMU-Pose
        with open(os.path.join(outputpath,file_name+'.json'), 'w') as json_file:
