...
class DataWriter():
    ...
    def update(self):
        ...
        while True:
            ...
            if len(self.opt.video) and orig_img is None:
                # if the thread indicator variable is set (img is None), stop the thread
                if self.save_video:
                    stream.release()
                write_json(final_result, self.opt.outputpath, self.opt, form=self.opt.format, for_eval=self.opt.eval)
                print("Results have been written to json.")
                return
            elif (not len(self.opt.video)) and len(final_result):
                write_json(final_result, self.opt.outputpath, self.opt, form=self.opt.format, for_eval=self.opt.eval)
                print("Results have been written to json.")
                final_result = []
                if orig_img is None:
        ...
        while True:
            ...
            if boxes is None or len(boxes) == 0:
                ...
                # if not self.opt.pose_track:
                    # boxes, scores, ids, preds_img, preds_scores, pick_ids = \
                        # pose_nms(boxes, scores, ids, preds_img, preds_scores, self.opt.min_box_area)
                preds_img_index = torch.from_numpy( np.array( [ k for k in range(hm_data.shape[1]) if k%3 != 0] , dtype = np.int64))
                # print('preds_img_index',preds_img_index)
                preds_img    =  torch.cat( ( hm_data[:,0::3].unsqueeze(2) , hm_data[:,1::3].unsqueeze(2) ) , dim =2 )
                preds_scores =  hm_data[:,2::3].unsqueeze(2)

                # print('scores' , scores)
                # print('len(scores)' , len(scores))
                # print('preds_scores.shape',preds_scores.shape)
                # print('preds_img',preds_img)
                # print('preds_img',preds_img.shape)
                scores = torch.mean(preds_scores, dim = 1)
                _result = []
                for k in range(len(scores)):
                    _result.append(
                        {
